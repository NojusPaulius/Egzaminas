...
const PostPage = () => {
  ...
  return (
    <section className="container d-flex flex-column align-items-center w-100">
      ...
      {((post.creator
        ? post.creator.username === localStorage.getItem("username")
        : false) ||
      ...
      <div className="mb-5">
        ...
        <a
          className="d-inline fs-4 m-1"
          href={`/user/${post.creator ? post.creator._id : ""}`}
        >
          {post.creator ? post.creator.username : "Unknown User"}
        </a>
        <p className="d-inline fs-4 m-1">
          {post.category ? post.category.category : "None"}
        </p>
      </div>
      <div className="w-75 text-center">
        <img className="w-100" src={post.image} alt={post.image} />
        <div className="w-100 my-5 fs-2" style={{ height: "auto" }}>
          {post.description}
        </div>
      </div>
      <button className="btn btn-primary mb-3">{post.price} eur</button>
      <div>
        <span onClick={handleLike} style={{ color: like, fontSize: "40pt" }}>
          ♥
        </span>{" "}
        {likes}
      </div>
      ...
      <article className="w-100">
        {post.comments && (
          <Comments comments={post.comments} loading={setLoading} />
        )}
      </article>
    </section>
  );
